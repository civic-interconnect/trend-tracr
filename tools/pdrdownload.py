...
#
def_pdrid = ""  # Insert ID to customize to a particular dataset, e.g. "mds2-3325"
def_filelist_url = None
description = f"""download data from the NIST PDR dataset, {def_pdrid}"""
...
def set_options(progname, args, parser=None):
    ...
    if not opts.pdrid:
        if not def_pdrid:
            raise MortalError(
                "No default dataset ID set; you must specify it with -I", 2
            )
    ...
    if opts.pdrid.startswith("https://"):
        ...
        try:
            ...
            if not urlpath.startswith("/od/id/") or len(urlpath) <= len("/od/id/"):
                raise MortalError(
                    "Unrecognized URL form given for dataset identifier: ",
                    2,
                )
...
opts = set_options(def_progname, ["-I", "noidea"])


def complain(message, exitwith=-1, ostrm=sys.stderr):
    """
    conditionally emit an error message, depending on a given verbosity level
...
def run():
    ...
    if not os.path.exists(opts.destdir):
        ...
        if not os.path.isdir(parent):
            raise MortalError(
                "Output parent dir is not an existing directory: " + parent, 2
            )
    ...
    try:
        if opts.verbosity > QUIET:
            ...
            print()
            (found, todo) = summarize_todo(
                filelist, opts.destdir, opts.select, opts.dodownload or not opts.docheck
            )
            print()
    except OSError as ex:
        fname = os.path.join(
            os.path.basename(os.path.dirname(filelist)), os.path.basename(filelist)
        )
        raise MortalError(f"problem reading file list ({fname}): {ex!s}", 3)
    ...
    if opts.dodownload or opts.docheck:
        ...
        if opts.verbosity > QUIET:
            ...
            print(
                "Beginning %s of %i files..."
                % (process, todo if opts.dodownload else found)
            )
        dlcount, failed = process_files(
            opts.dodownload,
            filelist,
            opts.destdir,
            select,
    ...
    if opts.verbosity > QUIET and not opts.dodownload:
        print(
            "Add the -D argument to actually download the %sdata."
            % ("selected " if opts.select else "")
        )
        if not opts.docheck:
            print(
                "Add the -C argument to just do a checksum check on the files downloaded so far."
            )
...
def ensure_filelist(listfile, destdir):
    ...
    try:
        ...
        raise MortalError(
            f"Failed to retrieve file list from {listfile}: {ex.reason} ({ex.code})",
            3,
        )
    except URLError as ex:
        raise MortalError(
            f"Failed to retrieve file list from {listfile}: {ex.reason}", 3
        )
...
def get_filelist(url, destdir):
    global opts
    if opts.verbosity >= VERBOSE:
        print(f"Fetching the list of files into {destdir}...")

    with urlopen(url) as resp:
        if (
            resp.info().get_content_type() == "application/json"
            or resp.info().get_content_type() == "application/ld+json"
        ):
    ...
    if os.path.basename(destfile) == "_nerdm.json":
        with open(destfile) as fd:
            try:
                ...
                raise MortalError(
                    "Unable to read NERDm resource metadata: " + str(ex), 2
                )
...
def get_default_filelist(pdrid, destdir):
    ...
    try:
        ...
        raise MortalError(
            "Unable to retrieve a metadata record to create file listing: " + str(ex), 2
        )
...
def summarize_todo(listfile, destdir, select=True, fordownload=True):
    ...
    with open(listfile) as fd:
        ...
        for row in rdr:
            ...
            fileselected = False
            if (
                isinstance(requested, set)
                and fname in requested
                or any([fname.startswith(r + "/") for r in requested])
            ) or requested == "All":
                fileselected = True
    ...
    if totsz == 0:
        print(
            f"  lists {total} file{'s' if total != 1 else ''}, {found} "
            f"of which {'are' if found != 1 else 'is'} already downloaded."
        )
        print("  (Warning: file table appears to not include proper file sizes.)")
    else:
        print(
            "  lists %i file%s (%s), %i (%s) of which %s already downloaded."
            % (
                total,
                "s" if total != 1 else "",
                formatBytes(totsz),
    ...
    if select:
        ...
        if totsz == 0:
            print(
                "%i file%s selected to be %s"
                % (todo, "s" if todo != 1 else "", process)
            )
        else:
            print(
                "%i file%s (%s) selected to be %s"
...
def summarize_done(diddownload, successcount, failedcount, failedfile=None):
    ...
    if diddownload:
        print(
            "Successfully downloaded %i file%s"
            % (successcount, "s" if successcount != 1 else "")
        )
    else:
        print(
            "%i downloaded file%s passed integrity checks"
            % (successcount, "s" if successcount != 1 else "")
        )
    if failedcount > 0:
        print(
            "%d file%s failed to download correctly"
            % (failedcount, "s" if failedcount != 1 else "")
        )
        if failedfile:
            print(f"  See {failedfile} for a list of failed downloads")
    return failedcount
...
def download_files(
    ...
    return process_files(
        True, listfile, destdir, select, failedtbl, docheck, rmonerr, force
    )
...
def process_files(
    ...
    if failedtbl:
        ...
        if same:
            MortalError(
                (
                    "Can't read files from failed output file\n"
                    "  Rename {} to retry previously failed files (or use -e or -E)"
                ).format(
                    os.path.join(
                        os.path.basename(os.path.dirname(failedtbl)),
                        os.path.basename(failedtbl),
                    )
                ),
                2,
            )
        ...
        with open(listfile) as fd:
            ...
            for row in tbl:
                if row[COL_FILE].lstrip().startswith("#"):
                    ...
                    continue
                if (
                    hasattr(select, "__contains__")
                    and row[COL_FILE] not in select
                    and not any(
                        row[COL_FILE].startswith(s.rstrip("/") + "/") for s in select
                    )
                ):
                ...
                if dodownload and not force and os.path.exists(destfile):
                    if opts.verbosity >= VERBOSE:
                        print(f"  skipping {row[COL_FILE]}; already downloaded")
                    continue
                ...
                if opts.verbosity > QUIET:
                    ...
                    sz = f" ({formatBytes(size)})" if size else ""
                    print(f"  {doing} {row[COL_FILE]}{sz}...{sp}", end=end)
                ...
                if size is None:
                    if opts.verbosity > SILENT:
                        complain(
                            "Warning: no size given in file table for " + row[COL_FILE]
                        )
                ...
                if not reason and docheck:
                    if not row[COL_HASH]:
                        if opts.verbosity > SILENT:
                            complain(
                                "Warning: no checksum hash in file table for "
                                + row[COL_FILE]
                            )
...
def nerdm_to_filelist(nerdm, listfile=None):
    ...
    if not isinstance(nerdm, Mapping):
        raise TypeError(
            "Input data does not look like a NERDm Resource: not a dictionary"
        )
    if "components" not in nerdm and "title" not in nerdm:
        raise ValueError(
            "Input data does not look like a NERDm Resource: no components nor title"
        )

    return nerdmcomps_to_filelist(
        nerdm.get("components", []), listfile, nerdm.get("@id"), nerdm.get("title")
    )
...
def nerdmcomps_to_filelist(nerdm, listfile=None, dsid=None, title=None):
    ...
    if os.path.exists(listfile):
        complain(
            "Warning: over-writing "
            + os.path.join(
                os.path.basename(os.path.dirname(listfile)), os.path.basename(listfile)
            )
        )
        ...
        for comp in nerdm:
            ...
            if comp.get("downloadURL") and (
                ...
                if not filep:
                    if opts.verbosity >= VERBOSE:
                        complain(
                            "warning: missing filepath property for url="
...
def main(progname, args):
    ...
    if failed > 0:
        raise MortalError(
            "%d file%s failed to download" % (failed, "s" if failed != 1 else ""), 2
        )
